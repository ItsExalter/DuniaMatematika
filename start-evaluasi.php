...
<body>
    <header>
        <nav class="navbar bg-white fixed-top">
            <p class="fs-4 text-center w-100 fw-bold">Evaluasi Koordinat Kartesius</p>
        </nav>
    </header>
    <main class="blur">
        ...
        <div class="bg-white scroll-style d-flex flex-column align-items-center justify-content-start rounded rounded-4 overflow-y-scroll w-100 h-75 latihan">
            <form class="py-4" action="result.php" method="post">
                <div class="d-flex flex-column align-items-start px-5">
                    ...
                    <input type='hidden' name="question-type" value="evaluasi"/>
                    <?php
                         $query = "SELECT * FROM `question-bank` WHERE question_type = 'evaluasi' ORDER BY RAND()";
                ...
                <div class="w-100 px-2 d-flex align-items-center justify-content-center">
                    <button class="btn py-1 px-3 fs-6 fw-bold border border-dark border-3" type="submit" id="submitEval">Kumpulkan Jawaban</a>
                </div>
                </div>
            </form>
        </div>
